...
class calculate_overtime_daily__overtime_worked_hours(Variable):
    ...
    def formula(persons, period, parameters):
      condition = persons("daily_work_schedule__is_holiday",period) + persons("work_category__daily_is_majority_highway_operator", period) + persons("work_category__daily_is_highway_operator_only", period)
      return where(
          (condition), 
